...
public class PopupControllerB extends RootController {
    ...
    public void onConfirm(){
        ...
        Platform.runLater(()->{
            taLogs.appendText(DateFormat.FORLOG.format(new Date())+"矩阵1即将遍历操作的通道为："+ Arrays.toString(channels)+"\n");
            System.out.println("矩阵1即将遍历操作的通道为："+text);
        });
    }
    public void onSwitchOn(){
        ...
        String echo=matrix1.channelSwitch(cmdChannels);
        Platform.runLater(()->{
            taLogs.appendText(DateFormat.FORLOG.format(new Date())+echo+"\n");
        });
    }
    public void onReset(){
        String echo = matrix1.rst();
